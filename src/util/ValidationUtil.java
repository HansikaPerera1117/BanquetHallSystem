...
public class ValidationUtil {
    ...
    private static void removeError(TextField txtField,JFXButton btn) {
     txtField.setStyle("-fx-text-fill: green; -fx-control-inner-background:  #fff7d2  ; -fx-background-radius:20");
        btn.setDisable(false);
    }

    private static void addError(TextField txtField,JFXButton btn) {
        if (txtField.getText().length() > 0) {
         txtField.setStyle("-fx-text-fill: red ; -fx-control-inner-background:  #fff7d2 ; -fx-background-radius:20");
            btn.setDisable(true);
        }
        btn.setDisable(true);
    }
